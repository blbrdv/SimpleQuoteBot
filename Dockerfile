...
#RUN /usr/bin/uuidgen -r >> /etc/machine-id
#
...
RUN sed -i 's/deb.debian.org/debian.anexia.at/g' /etc/apt/sources.list
RUN apt-get update && apt-get install -y wget
RUN wget https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb
RUN dpkg --force-all -i ./google-chrome*.deb || true  # fixing deps on next line
RUN apt-get install -y -f
